"""Publish one already-tested Windows installer/checksum pair with GitHub CLI."""
...
class GhReleaseOperations:
    ...
    def _release(value: dict) -> Release:
        ...
        return Release(tag, draft, frozenset(names))
    ...
    def download(self, tag: str, name: str, directory: Path) -> Path:
        self._run("release", "download", tag, "--repo", self.repository, "--pattern", name, "--dir", str(directory))
        path = directory / name
        if not path.is_file():
...
def publish_tested_pair(
    ...
    if final is None or not final.draft or final.tag != tag:
