...
def test_rename_cannot_rename_after_save(
    ...
    assert tab.display_name == "saved.pdf"
    assert tab.display_path == str(output)
    assert tab.suggested_save_stem() == "saved"
    assert tab.original_path in tab.identity_tooltip
